...
func _ready() -> void:
	...
	player.health_updated.connect(func(new_health : int): health.text = "Health: [color=%s]%02d[/color]" % ["green" if new_health > 10 else "red", new_health])
	health.text = "Health: [color=%s]%02d[/color]" % ["green" if player.current_health > 10 else "red", player.current_health]
